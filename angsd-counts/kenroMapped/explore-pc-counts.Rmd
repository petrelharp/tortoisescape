...
We'll make maps of all the SNPs that have correlation with PC1 above 0.95 
(there are `r sum(abs(pc1.df$snp.cor[goodones])>0.95)` of these).
```{r high_cor_maps, include=FALSE, cache=TRUE}
pc.num <- 1
high.cor.lines <- which( goodones & abs(pc1.df$snp.cor)>0.95 )
...
pc.num <- 2
high.cor.lines <- which( goodones & abs(pc1.df$snp.cor)>0.5 )
thisdir <- sprintf("cor_pc%d_gt_0.5_maps",pc.num)
dir.create(thisdir,showWarnings=FALSE)
png( filename=file.path(thisdir,"snp_map%03d.png"), 
        width=5*144, height=5*144, pointsize=10, res=144 )
