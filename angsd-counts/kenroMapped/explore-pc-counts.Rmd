...
scaf.len <- scaf.info$length[match(levels(pos$chr),scaf.info$chr)]
scaf.breaks <- c(0,cumsum(as.numeric(scaf.len)))
scaf.pos <- scaf.breaks[-1]-diff(scaf.breaks)/2
plotpos <- function (chrom,pos) { return(scaf.breaks[chrom]+pos) }
scafplot <- function (y,
        pch=20, cex=0.5,
    ...
    xx <- plotpos(pos$chr[use.these],pos$pos[use.these])
    yy <- y[use.these]
...
zz <- ifelse(goodones & abs(pc1.df$snp.cor)>0.5, abs(pc1.df$snp.cor), NA)
scafplot( zz, ylim=c(0,1), ylab='abs(correlation)', main='correlation with PC1')
...
scafplot( zz, ylim=c(0,1), ylab='abs(correlation)', main='correlation with PC2')
# points( scaf.pos/1e6, tapply(zz,pos$chr,mean,na.rm=TRUE), col='orange', cex=0.25 )
