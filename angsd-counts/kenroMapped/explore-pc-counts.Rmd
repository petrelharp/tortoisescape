...
It doesn't look like even alleles with correlation around 0.4 are particularly striking.


# Maps of high-correlation SNPs

We'll also make maps of all the SNPs that have correlation above 0.5 (there are `r sum(abs(pc.cor[goodones])>0.5)` of these`.
```{r high_cor_maps, include=FALSE}
high.cor.lines <- which( goodones & abs(pc.cor)>0.5 )
thisdir <- "cor_gt_0.5_maps"
dir.create(thisdir,showWarnings=FALSE)
png( filename=file.path(thisdir,"snp_map%03d.png"), 
        width=5*144, height=5*144, pointsize=10, res=144 )
