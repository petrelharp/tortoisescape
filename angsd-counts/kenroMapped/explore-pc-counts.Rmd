...
bincount <- file(countfile,open="rb")
...
map_alleles <- function (lines,...) {
    ...
    freqs <- tapply( 1:ncol(counts), 1:ncol(counts)%%4, function (k) { rowSums(sweep(counts,1,rowSums(counts),"/")[,k]) } )
    site.info <- pos[lines,]
    for (site in 1:nrow(counts)) {
        player( main=paste(site.info$chr[site],site.info$pos[site]), ... )
        mtext(side=3,line=0.5,text=paste(
                 "depth:",rowSums(counts)[site],
                 "freq:",max(freqs[site,]),
                 "nInd:",rowSums(!is.na(counts))[site]) )
