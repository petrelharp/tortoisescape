...
```{r cor_vs_freq, fig.width=2*fig.dim, cache=TRUE, depends="pc_count_connection"}
layout(t(1:2))
...
```{r long_scafs, cache=TRUE, depends="pc_count_connection"}
...
```{r high_cor_maps, include=FALSE, cache=TRUE}
pc.num <- 1
...
```{r high_cor_maps_2, include=FALSE, cache=TRUE}
pc.num <- 2
...
```{r all_snps, cache=TRUE}
plot( pos$totDepth, pc1.df$snp.cor, pch=20, cex=0.5,
     col=adjustcolor( ifelse( ( ( pos$totDepth >= mindepth ) & ( pos$totDepth <= maxdepth ) ),
                     ifelse(pos$chr %in% long.scafs, "black", "blue"), "red" ) ,0.5),
...
We'll also make some maps of the high-correlation,
non-filter-passing alleles.
There are `r sum(abs(pc2.df$snp.cor[goodones])>0.5)` of these).
```{r high_cor_maps_unfiltered, include=FALSE, cache=TRUE}
pc.num <- 1
high.cor.lines <- which( goodones & abs(pc1.df$snp.cor)>0.85 )
thisdir <- sprintf("cor_pc%d_gt_0.5_maps",pc.num)
dir.create(thisdir,showWarnings=FALSE)
