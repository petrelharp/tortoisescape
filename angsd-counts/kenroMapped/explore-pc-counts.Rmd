...
# Maps of high-correlation alleles
...
map_alleles <- function (lines,...) {
    ...
    freqs <- do.call( cbind, tapply( 1:ncol(counts), 1:ncol(counts)%%4, function (k) { rowSums(sweep(counts,1,rowSums(counts),"/")[,k]) } ) )
    ...
    for (site in 1:nrow(counts)) {
        ...
        mtext(side=3,line=0.5,
              text=sprintf("depth: %d, freq: %0.3f, nInd: %d, pc1cor: %0.3f",
                 ...
                 rowSums(!is.na(counts))[site], # num indivs
...
```
```{r map_scafs_list, results="asis"}
paste0("- [", superlong, "](", superlong, "_maps/centered/index.html", ")")
```
