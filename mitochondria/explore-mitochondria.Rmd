...
Let's look at those `r sum(rowSums(major.allele != total.major.allele) == 1)` sites where only one individual had a different major allele.
...
plot(singleton.coverages,main="major/minor coverages at singleton sites")
```
These all look pretty well supported, actually.
...
`r nondups <- (rowSums(minors)/rowSums(coverage)<.01); mean(nondups)`
of the sites,
...
abline(0, muhat)
matplot( cov.vals, quants, lty=3, type='l', add=TRUE )
```
That some of the `r nsamples` individuals fall outside of these quantiles
indicates that there is significant heterogeneity in error rate.
