...
if (!$inFile or !$outDir or !$mafsFile or !$minMAF or !$numLoci or $help) {
...
while ($mafsGZ->gzreadline(my $mafLine) > 0) {
    ...
    if ($mafLine =~ /^scaffold_(\d+)\t(\d+)\t[ATCG]\t[ATCG]\t0\.(\d+)/) {
        ...
        my $locus = $1 . "_$2";
        if ($3 > $minMAF) {
            $variableSitesHash{$locus}++; # This initializes a hash key that contains the scaffold and base pair of the variable site with the value 1 (we just want the hash keys for later)
        }
    }
}
...
### Now that we've stored all the variable sites above our minimum allele frequency cutoff, we'll randomly draw $numLoci hash keys from %variableSitesHash to give us the sites that
