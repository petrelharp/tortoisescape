...
if (file.exists("../torts-info.RData")) {
    ...
    diag(pimat) <- NA
    ref.inds <- run.info[[1]]$trust.optim$ref.inds
...
# set up plotting
tmp.config <- read.json.config(file.path("..",run.info[[value.order[1]]]$trust.optim$config.file))
load(file.path(run.info[[value.order[1]]]$subdir,tmp.config$setup_files))
tort.loc.obj <- load(file.path(run.info[[value.order[1]]]$subdir,tmp.config$sample_locs))
...
for ( k in value.order ) {
    if (length(run.info[[k]])>1) {
        ...
        abline(0,1)
        ph( run.info[[k]]$stationary.base, main=subdir )
        points(tort.locs,cex=abs(scale(mean.resids)),col=ifelse(mean.resids>0,"red","blue"))
