...
for ( k in value.order ) {
    if (length(run.info[[k]])>1) {
        # set up plotting
        tmp.config <- read.json.config(file.path("..",run.info[[value.order[k]]]$trust.optim$config.file))
        load(file.path(run.info[[value.order[k]]]$subdir,tmp.config$setup_files))
        tort.loc.obj <- load(file.path(run.info[[value.order[k]]]$subdir,tmp.config$sample_locs))
