...
Here's the observed hitting times against geographic distance:
...
for ( k in value.order ) {
    if (length(run.info[[k]])>1) {
        ...
        layout(t(1:2))
        plot( as.vector(sym.hts), pimat, col=adjustcolor(ifelse(used,'red','black'),0.25), pch=20, cex=0.5, ylab="observed times", xlab="fitted times", main=subdir )
        ...
        plot( as.vector(sym.hts), pimat, col=adjustcolor(ifelse(used,'red','black'),0.25), 
            pch=20, cex=0.5, ylab="observed times", xlab="fitted times", main=subdir,
