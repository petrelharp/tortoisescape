...
ph <- plot.ht.fn(file.path(dirname(trust.optim$config.file),config$layer_prefix),nonmissing,homedir="..",default.par.args=list(mar=c(3,3,3,3)+.1))
...
layout(t(1:2))
plot( as.vector(sym.hts), pimat, col=adjustcolor(ifelse(col(pimat)%in%ref.inds,'red','black'),0.25), pch=20, cex=0.5, ylab="observed times", xlab="fitted times" )
