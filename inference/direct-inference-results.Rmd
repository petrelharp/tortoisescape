...
matplot( trust.optim$argpath[,1,drop=FALSE], type='l', ylab='shift parameter', xlab='step number', lty=c(1,rep(1,nlayers),rep(2,nlayers)) )
matplot( trust.optim$argpath[,-1], type='l', ylab='parameter values', xlab='step number', lty=c(1,rep(1,nlayers),rep(2,nlayers)) )
```
...
And, fitted hitting times against the values we're trying to fit:
```{r fit_hitting_times, echo=FALSE, fig.width=fig.dim}
plot( pimat, params[1]+hts[locs,], col=adjustcolor(ifelse(col(pimat)%in%trust.optim$ref.inds,'red','black'),0.5), pch=20, cex=0.5, xlab="observed times", ylab="fitted times" )
abline(0,1)
```
