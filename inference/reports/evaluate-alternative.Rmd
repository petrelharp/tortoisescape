...
summary.expr <- quote( 
    { 
        diffmat <- sub.alt.hts-sub.hts
        pos.diffmat <- pmax(0,diffmat)
        neg.diffmat <- pmin(0,diffmat)
        dim(diffmat) <- dim(pos.diffmat) <- dim(neg.diffmat) <- c( sum(alt.good.refs), sum(alt.good.refs) )
        nearby <- get.nearby( sub.hts, TRUE )
        diffmat[ !nearby ] <- NA
        pos.diffmat[ !nearby ] <- NA
        neg.diffmat[ !nearby ] <- NA
        ...
        data.frame(
            ...
            "references increased" = sum( rowMeans( diffmat, na.rm=TRUE ) > 0, na.rm=TRUE ),
...
all.xtab <- xtable( eval(summary.expr) )
all.xtab[,1] <- "all"
...
for (k in which(alt.good.refs)) {
    ...
    ph( truncate((alt.hts[,k]-hts[,k]),diff.zlims), zlim=diff.zlims, col=diff.cols, do.lims=FALSE, main="difference", legend.width=2 )
    points( ref.points[k], pch=8, cex=2 )
    points( ref.points[alt.good.refs], cex=4*abs((sub.alt.hts[,k]-sub.hts[,k])[alt.good.refs])/max(abs((sub.alt.hts[,k]-sub.hts[,k])[alt.good.refs]),na.rm=TRUE), col=ifelse((alt.hts[,k]-hts[,k])[alt.good.refs]>0,"red","blue") )
    plot( alt, col=adjustcolor('red',.8), add=TRUE, legend=FALSE )
    ph( truncate((alt.hts[,k]-hts[,k])/(1+hts[,k]),ratio.zlims), zlim=ratio.zlims, col=diff.cols, do.lims=FALSE, main="ratio", legend.width=2 )
    points( ref.points[k], pch=8, cex=2 )
    plot( alt, col=adjustcolor('red',.8), add=TRUE, legend=FALSE )
