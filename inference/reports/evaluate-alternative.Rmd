...
hts <- hitting.analytic(ref.neighborhoods,G,blocked=block.these)
```
then, identify the reference points whose median hitting time to the other points is larger than $2 \times 10^5$.
```{r inaccessible_refs}
accessible <- function (hts, ref.locs, cutoff=2e5) { return( apply(hts[ref.locs,],2,median,na.rm=TRUE) < cutoff ) }
...
isolated <- function (hts,ref.locs,accessible.refs,cutoff=2e5) { x <- rowMeans(hts[ref.locs,accessible.refs],na.rm=TRUE) > cutoff; return(ifelse(is.na(x),TRUE,x))  }
...
here is the mean difference in resistance distance for each point averaged across *the nearest 20%* of reference locations,
...
get.nearby <- function (hts,good.refs) {
    nearby <- ( sym(hts[ref.locs,]) < apply(sym(hts[ref.locs,]),1,quantile,.2) )  # average over nearest 20% of locations
    nearby[,!good.refs] <- FALSE
    return(nearby)
}
nearby <- get.nearby(hts,alt.good.refs)
...
plot( interp.values(mean.tdiffs,alt.good.refs), main="mean difference", col=diff.cols, zlim=zlims, legend.width=2 )
points( ref.points[alt.good.refs], cex=scale(abs(mean.tdiffs)), col=ifelse(mean.tdiffs>0,"red","blue") )
plot( interp.values(max.tdiffs,alt.good.refs), main="maximum difference", col=diff.cols, zlim=zlims, legend.width=2 )
points( ref.points[alt.good.refs], cex=scale(abs(max.tdiffs)), col=ifelse(max.tdiffs>0,"red","blue") )
```
...
- (**cells removed**) number of newly inaccessible or isolated raster cells in the alternative, or made inaccessible by it
- (**max increase**) the largest increase across reference locations of the commute time from the nearest 20% of reference locations
- (**max decrease**) the largest decrease across reference locations of the commute time from the nearest 20% of reference locations
- (**mean increase**) the mean increase across reference locations of the commute time from the nearest 20% of reference locations, with decreases counted as zero
- (**mean decrease**) the mean decrease across reference locations of the commute time from the nearest 20% of reference locations, with increases counted as zero
- (**relative increase**) the mean increase divided by the commute time without the alternative, with decreases counted as zero
- (**relative decrease**) the mean decrease divided by the commute time without the alternative, with increases counted as zero
```{r summary_fun,echo=FALSE}
summary.expr <- quote( 
    { 
        pos.diffmat <- pmax(0,sub.alt.hts-sub.hts)
        neg.diffmat <- pmin(0,sub.alt.hts-sub.hts)
        dim(pos.diffmat) <- dim(neg.diffmat) <- c( sum(alt.good.refs), sum(alt.good.refs) )
        pos.diffmat[ !nearby[alt.good.refs,alt.good.refs] ] <- NA
        neg.diffmat[ !nearby[alt.good.refs,alt.good.refs] ] <- NA
        ...
        data.frame(
            ...
            "max increase" = max( pos.diffmat, na.rm=TRUE ),
            "max decrease" = min( neg.diffmat, na.rm=TRUE ),
            "mean increase" = mean( pos.diffmat, na.rm=TRUE ),
            "mean decrease" = mean( neg.diffmat, na.rm=TRUE ),
            "relative increase" = mean( pos.diffmat/(1+sub.hts), na.rm=TRUE ),
            "relative decrease" = mean( neg.diffmat/(1+sub.hts), na.rm=TRUE )
    ) } )
...
xtab <- xtable(chunk.stats[order(chunk.stats$mean.increase,decreasing=TRUE),])
...
for (k in seq_along(chunk.info.list)[order(chunk.stats$mean.increase,decreasing=TRUE)]) {
    print( with( list2env( chunk.info.list[[k]] ), {
            cat("Chunk ", chunk.labels[k], ":\n")
            pos.diffmat <- pmax(0,sub.alt.hts-sub.hts)
            dim(pos.diffmat) <- c( sum(alt.good.refs), sum(alt.good.refs) )
            pos.diffmat[ !nearby[alt.good.refs,alt.good.refs] ] <- NA
            neg.diffmat[ !nearby[alt.good.refs,alt.good.refs] ] <- NA
            mean.tdiffs <- rowMeans(sym(pos.diffmat),na.rm=TRUE)
            usethese <- !is.na(mean.tdiffs)
            max.tdiffs <- apply(sym((sub.alt.hts-sub.hts)),1,max,na.rm=TRUE)
            plot( interp.values(mean.tdiffs[usethese],usethese&alt.good.refs), main="mean increase", col=diff.cols, zlim=zlims )
