...
```{r which_chunks, fig.width=2*fig.dim}
...
plot(alt, xlim=c(327250,777200), ylim=c(3.6e6,4.1e6), col=rainbow(max(chunk.labels)))
text( chunk.centers, labels=chunk.labels )
points( chunk.centers, pch=21, cex=3, col=chunk.cols )
for (xylims in chunk.xylims) {
    plot(alt, xlim=xylims[["xlim"]], ylim=xylims[["ylim"]], col=rainbow(max(chunk.labels)))
    text( chunk.centers, labels=chunk.labels )
    points( chunk.centers, pch=21, cex=3, col=chunk.cols )
}
```
...
for (k in seq_along(chunk.info.list)) {
    with( list2env( chunk.info.list[[k]] ), {
            mean.tdiffs <- rowMeans(sym(pmax(0,(sub.alt.hts-sub.hts))))
