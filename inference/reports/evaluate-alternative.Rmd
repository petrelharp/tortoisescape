...
nearby[!alt.good.refs,] <- nearby[,!alt.good.refs] <- FALSE
...
for (k in seq_along(chunk.info.list)[order(chunk.stats$mean.increase,decreasing=TRUE)]) {
    print( with( list2env( chunk.info.list[[k]] ), {
            ...
            mean.tdiffs <- rowMeans(sym(pos.diffmat))
            max.tdiffs <- apply(sym((sub.alt.hts-sub.hts)),1,max)
            plot( interp.values(mean.tdiffs,alt.good.refs), main="mean increase", col=diff.cols, zlim=zlims )
