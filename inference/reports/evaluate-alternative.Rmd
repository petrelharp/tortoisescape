...
pi.fac <- (( 56575857 / 1898838430 ) / 2.01e-9)/(.018 * 1e8)
```
...
accessible <- function (hts, ref.locs, cutoff=1e6) { return( apply(hts[ref.locs,],2,median,na.rm=TRUE) < cutoff ) }
...
isolated <- function (hts,ref.locs,accessible.refs,cutoff=1e6) { x <- rowMeans(hts[ref.locs,accessible.refs],na.rm=TRUE) > cutoff; return(ifelse(is.na(x),TRUE,x))  }
...
get.nearby <- function (sub.hts,good.refs,thresh=.2) {
