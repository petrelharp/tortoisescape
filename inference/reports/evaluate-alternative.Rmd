...
chunk.hts <- function (k,plot.points=rep(TRUE,length(ref.neighborhoods))) {
    ...
    new.hts <- hitting.analytic(ref.neighborhoods[plot.points],G,blocked=unique(c(chunk,blocked,block.these)))
...
ph( z<6,col=c('red','white'),legend=FALSE,main="isolated areas")
if (any(isolated.refs)) { points(ref.points[isolated.refs],cex=1.5,pch=21,col='blue',lwd=2) }
```
...
usethese <- ( sym(hts[ref.locs,]) < apply(sym(hts[ref.locs,]),1,quantile,.2) )  # average over nearest 20% of locations
usethese[!alt.good.refs,] <- usethese[,!alt.good.refs] <- FALSE
mean.tdiffs <- ( rowSums( sym((alt.hts-hts)[ref.locs,]) * usethese ) / rowSums(usethese) )[alt.good.refs]
max.tdiffs <- apply( sym((alt.hts-hts)[ref.locs,]) * usethese, 1, max )[alt.good.refs]
diff.cols <- diverge_hcl(128, h=c(225,0), c=100, l=c(60,95), power=0.65)
layout(t(1:2))
...
summary.expr <- quote( 
    { 
        ...
        data.frame(
            ...
            "max increase" = max( pos.diffmat ),
            "max decrease" = min( neg.diffmat ),
            "mean increase" = mean( pos.diffmat ),
            "mean decrease" = mean( neg.diffmat ),
            "relative increase" = mean( pos.diffmat/(1+sub.hts) ),
            "relative decrease" = mean( neg.diffmat/(1+sub.hts) )
    ) } )
