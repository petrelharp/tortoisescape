...
ref.diffs <- sapply( seq_along(footprints), function (k) { rowMeans( diff.ht.list[[k]][locs[do.ref.inds[[k]]],] ) } )
zlims <- inflate( ref.diffs )
layout( matrix(1:25,nrow=5,ncol=5) )
for (k in seq_along(footprints)) {
    ph( rowMeans( diff.ht.list[[k]] ), par.args=list(mar=c(0,0,0,0)+.1), zlim=zlims, legend=FALSE )
}
```
