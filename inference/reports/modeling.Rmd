...
inflate <- function (...,fac=.2) { xr <- range(...); mean(xr) + (1+fac) * (xr-mean(xr)) }
```
...
footprint.centers <- expand.grid( x=seq(-17e5,-19e5, length.out=5), y=seq(-2e5,-4e5,length.out=5) )
footprints <- get.neighborhoods( 15e3, footprint.centers, nonmissing, layer )
ph( ifelse(1:nrow(layers)%in%unlist(footprints),1,0), do.lims=FALSE, main="removed bits" )
```
and compute hitting times
```{r many_footprints_hts, fig.width=4*fig.dim, fig.height=4*fig.dim}
zeroed.ht.list <- lapply( footprints, function (fp) { 
        do.ref.inds <- ref.inds[ ! locs[ref.inds] %in% fp ]
        hitting.analytic(neighborhoods[do.ref.inds],G,blocked=fp) 
    } )
diff.ht.list <- lapply( zeroed.ht.list, "-", hts[,ref.inds] )
ref.diffs <- sapply( diff.ht.list, function (x) { rowMeans( x[locs[ref.inds],] ) } )
zlims <- inflate( ref.diffs )
layout( matrix(1:25,nrow=5,ncol=5)[,5:1] )
for (k in seq_along(footprints)) {
    ph( rowMeans( diff.ht.list[[k]] ), par.args=list(mar=c(0,0,0,0)+.1) )
}
```
