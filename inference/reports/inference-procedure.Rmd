...
Well, the first thing that happens in a numerical error
```{r singular_LU}
...
wrong.interp.hts <- interp.hitting( neighborhoods[ref.inds[check.inds]], G, true.obs.hts[check.inds,check.inds], obs.locs=locs[ref.inds[check.inds]], alpha=0, numcores=1 )
```

Since $\alpha=0$, the interpolation problem has the same answer as the hitting time problem.
```{r fix_singular_LU}
small.jump.rates <- which( rowSums(G)<1e-20 )
wrong.interp.hts <- interp.hitting( neighborhoods[ref.inds[check.inds]], G, true.obs.hts[check.inds,check.inds], obs.locs=locs[ref.inds[check.inds]], alpha=0, numcores=1, blocked=small.jump.rates )
```
