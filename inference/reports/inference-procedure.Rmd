...
Well, the first thing that happens is a numerical error
```{r singular_LU}
...
```
Since $\alpha=0$, the two answers should be the same.

```{r fix_singular_LU}
small.jump.rates <- which( rowSums(G)<1e-8 )
try( { wrong.interp.hts <- interp.hitting( neighborhoods[ref.inds[check.inds]], G, true.obs.hts[check.inds,check.inds], obs.locs=locs[ref.inds[check.inds]], alpha=0, numcores=1, blocked=small.jump.rates ) } )
length(G[-1,1]) # run this to avoid segfault after "computationally singular" error
klocs <- neighborhoods[[1]]
GG <- crossprod( (G-Diagonal(nrow(G),rowSums(G)))[-klocs,-klocs] )
tmp <- solve(GG, rep(-1,nrow(GG)))
length(G[-1,1]) # run this to avoid segfault after "computationally singular" error
```
...
```{r try_symm, fig.width=fig.dim, fig.height=fig.dim, cache=TRUE}
...
plot( orig.hts, sym.hts, log='xy'); abline(0,1)
```
That checks out.  Now, let's plug $J$ in for $G$ in the interpolation routine:
```{r interp_sym, fig.width=fig.dim, fig.height=fig.dim, cache=TRUE}
sym.bvec <- as.vector( crossprod( (J-Diagonal(nrow(G),rowSums(G)))[-klocs,-klocs], nu[-kn] ) )
sym.hts.2 <- numeric(nrow(G))
sym.hts.2[-kn] <- as.vector( solve( crossprod( (J-Diagonal(nrow(G),rowSums(G)))[-klocs,-klocs] ), sym.bvec ) * sqrt(pivec[-kn]) )
range( ((G-Diagonal(nrow(G),rowSums(G)))%*%sym.hts.2)[-kn] )
plot( orig.hts, sym.hts.2, log='xy'); abline(0,1)
```
Looks good!
