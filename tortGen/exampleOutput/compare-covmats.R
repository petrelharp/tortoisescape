...
rcov <- matrix( scan("alleleCounts500kLoci-covmat.txt"), nrow=180 )

layout(t(1:2))
plot(as.vector(subcov),as.vector(rcov)); abline(0,1)
plot(as.vector(subcov[lower.tri(subcov,diag=FALSE)]),as.vector(rcov[lower.tri(subcov,diag=FALSE)])); abline(0,1)
