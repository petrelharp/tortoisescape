...
my %tortReadDepthHash;


# Gather the read depth values
while (my $line = <>) { # Read from STDIN
    ...
    foreach my $tortNum (@etortNumbers) {
        my $tort = "etort-" . $tortNum;
        my @individualDepths = splice(@fields,0,4); # Remove the first 4 elements of the array 
        my $locusDepth = sum(@individualDepths);        
        $tortReadDepthHash{$tort}{$locusDepth}++;        
        
    }
}

#print Dumper(\%tortReadDepthHash);


# Print out the header line
print "Individual\t";
foreach my $depth (1..900) {
    print $depth . "read\t"
}
print "\n";


# Print all the read depth values
foreach my $tort (@etortNumbers) {
    my $tortName = "etort-$tort";
    print $tortName . "\t";
    foreach my $depth (1..900) {
        if (exists $tortReadDepthHash{$tortName}{$depth}) {
            print $tortReadDepthHash{$tortName}{$depth} . "\t";    
        } else {
            print "0\t";
        }
