...
names(C) <- gsub("C.etort1","",names(C))

plot( C, torts$coverage[match(names(C),torts$EM_Tort_ID)] )

layout(t(1:2))
with(dists, plot( DISTANCE, pi ) )
with(dists, plot( A*DISTANCE, npi ) )

layout(t(1:2))
with(dists, {
        plot( DISTANCE, type='n' )
        text( DISTANCE, labels=gsub("etort.","",dists$etort1) )
    })
with(dists, {
        plot( DISTANCE, type='n' )
        text( DISTANCE, labels=gsub("etort.","",dists$etort2) )
    })
