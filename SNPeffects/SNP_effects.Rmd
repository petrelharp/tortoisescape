...
scaf_cor_colors <- adjustcolor(ifelse(indivs$scafPC>scaf_cor_cutoff,'blue','red'),0.5)
...
Here is coverage on the top few, plotted in a few different ways:
```{r wierd_scaffold}
...
show_scaf <- function (this_coverage) {
    image(1:nrow(this_coverage), z=this_coverage, xlab='SNP number', ylab='individual', yaxt='n')
    plot(row(this_coverage), col(this_coverage), cex=this_coverage/colMeans(this_coverage)/10, col=adjustcolor('black',0.5), xlab='SNP number', ylab='individual', yaxt='n')
    invisible(this_coverage)
}
scafs <- lapply(scaffolds$name[head(order(scaf_pcs$vectors[,1], decreasing=TRUE))], get_coverage)
layout(t(1:2))
for (ts in scafs) { show_scaf(ts) }
```

Here's some scaffolds from the middle of the pack, for comparison:
```{r middle_scaffolds}
mid_scafs <- lapply(scaffolds$name[order(scaf_pcs$vectors[,1], decreasing=TRUE)[500:504]], get_coverage)
layout(t(1:2))
for (ts in mid_scafs) { show_scaf(ts) }
```
