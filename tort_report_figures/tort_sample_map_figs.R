...
load("../visualization/county_lines.Robj")
...
png(file="sample.map.png",res=200,width=10*200,height=5*200)
...
png(file="tort_report_figures/black_and_white_272_sample.map.png",res=200,width=10*200,height=5*200)
#quartz(width=10,height=5)
par(mfrow=c(1,2))
...
plot(dem,ylab="",xlab="",main="Tortoise Sample Map",xaxt='n',yaxt='n')
		polygon(x=c(x.min,x.max,x.max,x.min,x.min),y=c(y.min,y.min,y.max,y.max,y.min),lty=2,lwd=0.5)
		#points(tort.coords[match(c("etort-1","etort-16","etort-70"),row.names(tort.coords)),],pch=1,col="green",cex=2)
			lines(county_lines,lwd=0.5)
			lines(state.lines.spobj,lwd=2)
		points(tort.coords,pch=tort.plotting.pch,cex=0.7,col=tort.plotting.colors.discrete)
			box(lwd=3)
...
plot(dem,ylab="",xlab="",main="Tortoise Sample Map",xaxt='n',yaxt='n')
		...
		#points(tort.coords[match(c("etort-1","etort-16","etort-70"),row.names(tort.coords)),],pch=1,col="green",cex=2)
			lines(county_lines,lwd=0.5)
			lines(state.lines.spobj,lwd=2)
		points(tort.coords,pch=tort.plotting.pch,cex=0.7,col=tort.plotting.colors.continuous)
			box(lwd=3)
		rect(-2155000,-5.75e+05,-1872000,-4.2e+05,col="white")
		subplot(fun = {		plot(eig.covmat$vectors[,1],eig.covmat$vectors[,2],
...
plot(dem,ylab="",xlab="",main="Map of Tortoise Samples",xaxt='n',yaxt='n')
		points(tort.coords,pch=20,cex=0.7,col=tort.plotting.colors)
			lines(county_lines,lwd=0.5)
			lines(state.lines.spobj,lwd=2)
			box(lwd=3)
