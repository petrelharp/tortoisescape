...
params <- init.params
toJSON(params, file=file.path(outdir,"params.json"), pretty=TRUE)

dem <- do.call( model_setup, c( list(pop=pop), init.params ) )
...
plot( dist.df$pi, sim.dist, pch=20, cex=0.5,
     xlab="observed divergence", ylab="mean TMRCA (y)",
     col=adjustcolor(dist.df$col,0.5) )
dist.lm <- lm( sim.dist ~ dist.df$pi, subset= (dist.df$etort1 != dist.df$etort2) )
abline(coef(dist.lm))
...
for (tort in paste0("etort-",c(243,35,100,262,218,52,274,90,36))) {
    ...
    plot( dist.df$distance/1e3, dist.df$pi, pch=20, cex=0.5, 
         xlab="geog dist (km)", ylab="mean TMRCA (y)", main="observed" )
    points( dist.df$distance[ut]/1e3, dist.df$pi[ut], pch=20, cex=2,
         col=pc.cols[other,1] )
    plot( dist.df$distance/1e3, sim.dist, pch=20, cex=0.5,
         xlab="observed divergence", ylab="mean TMRCA (y)", main="simulated" )
